...
public class GererAssetsDragons : MonoBehaviour
{
    ...
    void Start()
    {
        // GameObject boutonsAnimations = GameObject.Find("Boutons Animations");

        // if (boutonsAnimations != null)
        // {
        //     Button[] buttons = boutonsAnimations.GetComponentsInChildren<Button>();
        // }

        // else
        // {
        //     Debug.LogWarning("Impossible de trouver les boutons d'animations.");
        // }
    }

    public void BoutonTrigger(Button boutonAnimation)
    {
        Vector3 playerV = Vector3.zero;

        switch (boutonAnimation.name)
        {
            case "Idle":
            case "Inactif":
                EffectClear();
            ...
            case "Aléatoire":
                ...
                Button boutonAnimationAleatoire = boutonsEnfants[Random.Range(0, boutonsEnfants.Length)].GetComponent<Button>();
                BoutonTrigger(boutonAnimationAleatoire);
                break;
